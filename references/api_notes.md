...
| `409 {"code":"AUTH_SESSION_LIMIT","message":"Conflict","success":false}` on `/api/user/login` | Session quota FULL: 50 active sessions per user (v1.0.0-rc.25), and sessions never auto-expire. Script logins that never log out pile up until the quota fills. | Clean stale sessions in the DB (`UPDATE user_sessions SET status='revoked', revoked_at=strftime('%s','now'), revoked_reason='cleanup' WHERE status='active' AND created_at < strftime('%s','2026-08-24 00:00:00');`) then `docker restart new-api`. Reuse access tokens; log out after scripted work. |
